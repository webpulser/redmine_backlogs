...
class RbMasterBacklogsController < RbApplicationController
  ...
  def index
    ...
    last_story = RbStory.find(
                          :first, 
                          :conditions => ["project_id in (?) AND tracker_id in (?)", @projects.map(&:id), RbStory.trackers],
                          :order => "updated_on DESC"
                          )
    @last_update = (last_story ? last_story.updated_on : nil)
    @product_backlog = { :sprint => nil, :stories => product_backlog_stories }
    @sprint_backlogs = sprints.map{ |s| { :sprint => s, :stories => s.all_stories } }
  ...
  def show
    ...
    @last_update = (last_story ? last_story.updated_on : nil)
    @product_backlog = { :sprint => nil, :stories => product_backlog_stories }
    @sprint_backlogs = sprints.map{ |s| { :sprint => s, :stories => RbStory.sprint_backlog_with_project(s, @project) } }
  ...
  def menu
    ...
    if @project = Project.find_by_id(params[:project_id])
      links << {:label => l(:label_task_board), :url => url_for(:controller => 'rb_taskboards', :action => 'show', :sprint_id => @sprint, :only_path => true, :project_id => @project.id)} if @sprint && @sprint.stories.size > 0
    else
      links << {:label => l(:label_task_board), :url => url_for(:controller => 'rb_taskboards', :action => 'index', :sprint_id => @sprint, :only_path => true)} if @sprint && @sprint.stories.size > 0
    end
